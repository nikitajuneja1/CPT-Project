...
function sendToCatBot(message_str){
    ...
    $listen_to_me_btn.text("Awaiting response from the foodbooker");
	g_ajaxer(G_API_GATEWAY_URL_STR, params, ok_cb, fail_cb);
}

function fail_cb(error){
	console.error(error);
	newBubble("gorgeous_catbot", "Failed");
	$listen_to_me_btn
			.text("Push to talk")
			.prop("disabled", false);
}
...
function ok_cb(response){
	...
	var 
		...
		opinion_str = "You can donate food";
...
// console.info(JSON.parse(response.body).dialogState, report_str);
	...
	if(response.dialogState === "Fulfilled"){ // "ReadyForFulfillment"){
		...
		message_str = "Ok, so you want to know if you can donate food in" 
				+ city_str
